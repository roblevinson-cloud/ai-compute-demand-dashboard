...
const capexDate=(p)=>{
  if(p.capex_effective_date)return p.capex_effective_date;
  const ev=(p.evidence||[]).find(e=>/(capex|construction commitment|development cost|financing|debt|notes|build cost|cost per)/i.test(e.headline||''));
  return ev?.date||'n/d';
};
let projects=[];
Promise.all([
...
function barRows(list,valKey,max,labelFn,live=false,showDate=false){if(!list.length||!max)return '<div class="pad sub">No comparable data.</div>';return list.map(p=>{const v=p[valKey];const liveV=num(p.live_mw)?p.live_mw:0;return `<a class="barrow ${showDate?'barrow-date':''} barlink" href="${projectHref(p)}" aria-label="Open ${p.name} project page"><div class="barname">${p.name}</div><div class="bartrack"><div class="barfill" style="width:${v/max*100}%"></div>${live?`<div class="barlive" style="width:${liveV/max*100}%"></div>`:''}</div><div class="barval">${labelFn(p)}</div>${showDate?`<div class="bardate">${capexDate(p)}</div>`:''}</a>`}).join('')}
function renderBars(){const byMW=projects.filter(p=>num(p.critical_it_mw)&&p.critical_it_mw>0).sort((a,b)=>b.critical_it_mw-a.critical_it_mw).slice(0,12);const byDebt=projects.filter(p=>num(p.debt_amount_b)&&p.debt_amount_b>0).sort((a,b)=>b.debt_amount_b-a.debt_amount_b).slice(0,12);const byCap=projects.filter(p=>num(p.capex_per_mw)&&p.capex_per_mw>0).sort((a,b)=>b.capex_per_mw-a.capex_per_mw).slice(0,12);el('capacityChart').innerHTML=barRows(byMW,'critical_it_mw',Math.max(...byMW.map(p=>p.critical_it_mw)),p=>mw(p.critical_it_mw),true)+`<div class="legend"><span><i class="dot cyan"></i>Tracked / contracted</span><span><i class="dot green"></i>Live</span><span>Click any project to open its deal page</span></div>`;el('debtChart').innerHTML=barRows(byDebt,'debt_amount_b',Math.max(...byDebt.map(p=>p.debt_amount_b)),p=>'$'+p.debt_amount_b.toFixed(p.debt_amount_b<1?2:1)+'B')+`<div class="legend">Known project debt; bank programs and bonds shown on the same scale. Click through for capital structure.</div>`;el('capexChart').innerHTML=`<div class="capexhead"><span>Project</span><span></span><span>$ / MW</span><span>Effective</span></div>`+barRows(byCap,'capex_per_mw',Math.max(...byCap.map(p=>p.capex_per_mw)),p=>text(p.capex_per_mw_display),false,true)+`<div class="legend">Effective = dated disclosure/source supporting the current capital-intensity observation where identified; n/d means the monitor still needs a dated source. We will preserve prior observations rather than overwrite them.</div>`}
