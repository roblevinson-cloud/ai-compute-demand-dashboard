...
function renderKPIs(){
  ...
  el('kpis').innerHTML=[
    ['TRACKED PROJECTS',projects.length,`${ig} IG / IG-linked · ${hy} HY`],
    ['CRITICAL IT',(total/1000).toFixed(2)+' GW','Known / disclosed capacity basis'],
    ['PROJECT DEBT','$'+debt.toFixed(1)+'B',`${loan} bank-loan programs · bonds + project loans`],
    ['WATCHLIST',watch+' projects','Power · permits · construction · credit']
...
function renderTable(rows){
 ...
 <td><div>${text(p.capex_per_mw_display)}</div><div class="sub">${text(p.capex_total_display)}</div></td>
 <td>${creditCell(p)}</td>
 <td>${debtCell(p)}</td>
...
document.querySelectorAll('#projectTable th').forEach(th=>th.onclick=()=>{
 const k=th.dataset.key;asc=k===sortKey?!asc:true;sortKey=k;
 const rows=[...projects].sort((a,b)=>{let x=a[k]??'',y=b[k]??'';if(typeof x==='number'&&typeof y==='number')return asc?x-y:y-x;return asc?String(x).localeCompare(String(y)):String(y).localeCompare(String(x))});
 renderTable(rows);
});