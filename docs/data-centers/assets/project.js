const root=document.getElementById('projectRoot'),slug=document.body.dataset.projectSlug;
...
Promise.all([
 ...
 const all=[
   ...base.projects.map(p=>({...p,...(overrides[p.slug]||{})})),
   ...(credit.projects||[]),
   ...(hy.projects||[])
 ];
 document.getElementById('asof').textContent='As of · '+(credit.as_of||hy.as_of||base.as_of);
 const p=all.find(x=>x.slug===slug);
 if(!p){root.innerHTML='<h1>Project not found</h1>';return}
 document.title=p.name+' — DC Intelligence';
 render(p);
});
...
function render(p){
 ...
 <section class="grid two section">
  <div class="card"><div class="cardhead"><strong>Deal & build economics</strong><span>Comparable operating fields</span></div>${facts([
   ['Developer / landlord',text(p.developer)+' · '+text(p.landlord)],
   ['Tenant / support',text(p.tenant)+' · '+text(p.tenant_detail)],
   ['Contracted revenue',text(p.contracted_revenue_display)],
   ['Critical / utility capacity',mw(p.critical_it_mw)+' / '+mw(p.utility_mw)],
   ['Capital intensity',text(p.capex_per_mw_display)+' — '+text(p.capex_note)],
   ['Delivery',text(p.delivery)]
  ])}</div>
  <div class="card"><div class="cardhead"><strong>Capital markets</strong><span>${text(p.credit_bucket)}</span></div>${facts([
   ['Instrument',text(p.financing_type,text(p.financing))],
   ['Debt amount',text(p.debt_amount_display)],
   ['Coupon / spread',text(p.coupon_display)],
   ['Maturity',text(p.maturity)],
   ['Rating',text(p.rating_display)],
   ['Issue price',text(p.issue_price_display)],
   ['Security',text(p.security_display)],
   ['Structure note',text(p.capital_markets_note,text(p.financing))]
  ])}</div>
 </section>
 <section class="grid two section">
  <div class="card"><div class="cardhead"><strong>Power stack</strong><span>${text(p.grid)}</span></div>${facts([
   ['Provider',text(p.power_provider)],['Source / infrastructure',text(p.power_source)],['Utility MW',mw(p.utility_mw)],['Cooling',text(p.cooling)]
  ])}</div>
